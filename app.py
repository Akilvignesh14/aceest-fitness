...
class ACEestApp:
    def __init__(self, root):
        self.root = root
        self.root.title("ACEest Fitness & Performance v2.2.4")
    ...
    def init_db(self):
        # Use in-memory for headless tests
        db_path = ":memory:" if not self.is_real_ui else DB_NAME
        self.conn = sqlite3.connect(db_path)
        self.cur = self.conn.cursor()

        # Schema Evolution: Tables for Clients, Progress, Workouts, Exercises, and Metrics
        self.cur.execute("""
    ...
    def setup_ui(self):
        # Header & Status
        header = tk.Label(self.root, text="ACEest Functional Fitness System v2.2.4", bg="#d4af37", fg="black", font=("Helvetica", 24, "bold"), height=2)
        header.pack(fill="x")
        ...
        # UI Construction gated by is_real_ui
    ...
    def _build_sidebar(self, parent):
        ...
        self.name = tk.StringVar(master=self.root)
        self.age = tk.IntVar(master=self.root); self.height = tk.DoubleVar(master=self.root)
        self.weight = tk.DoubleVar(master=self.root); self.program = tk.StringVar(master=self.root)
        ...
        ttk.Button(parent, text="Save Client", command=self.save_client).pack(pady=5)
    ...
    def _build_notebook(self, parent):
        ...
        nb.add(tab1, text="Client Summary")
    ...
    def save_client(self):
        ...
        w = self.weight.get()
        cal = int(w * self.programs[self.program.get()]["factor"]) if w > 0 else 0
        self.cur.execute("INSERT OR REPLACE INTO clients (name, height, weight, program, calories) VALUES (?, ?, ?, ?, ?)",
                         (self.name.get(), self.height.get(), w, self.program.get(), cal))
        self.conn.commit(); self.refresh_client_list(); messagebox.showinfo("Saved", "Client updated")

    def load_client(self):
        self.cur.execute("SELECT * FROM clients WHERE name=?", (self.name.get(),))
        row = self.cur.fetchone()
        if row:
            self.height.set(row[3]); self.weight.set(row[4]); self.program.set(row[5])
            self.refresh_summary()

    def refresh_summary(self):
        self.summary.config(state="normal")
        self.summary.delete("1.0", "end")
        self.summary.insert("end", f"CLIENT: {self.name.get()}\nPROGRAM: {self.program.get()}\nWEIGHT: {self.weight.get()}kg")
    ...
    def save_progress(self):
        ...
        self.conn.commit(); messagebox.showinfo("Logged", "Weekly progress saved")
    ...
    def show_progress_chart(self):
        ...
        if data:
            ...
            plt.title("Adherence Progress"); plt.show()
    ...
    def show_weight_chart(self):
        ...
        if data:
            plt.figure(figsize=(8,4))
            plt.plot([r[0] for r in data], [r[1] for r in data], color="orange")
            plt.title("Weight Trend"); plt.show()

    def open_log_workout_window(self):
        if not self.is_real_ui: return
        # Logic for workout popup...
        messagebox.showinfo("Workout", "Workout logging module active")
